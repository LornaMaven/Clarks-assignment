...
```{r}
ggplot(data = clarks.spend, aes(x = store.spend, y = online.spend)) +
          geom_point() +
...
Online only people seem to only buy one pair at a time - incentivise buy one get one 20% off ass they add to cart?
...
Store only people tend not to spend as much overall - can we get email addresses in store? Do we already ask? How high is take up rate?
...
Can see what you would expect here
