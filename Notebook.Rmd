...
```{r}
clarks.spend %>%
  ggplot( aes(x = online.profile, y = total.spend.per.trans, group = online.profile, color = distance.to.store)) + 
...
```{r}
clarks.spend[clarks.spend$sat.service != "NA",] %>%
...
```{r}
 ggplot(data = clarks.spend, aes(x = age, y = total.spend, colour = online.profile)) +
        ...
        scale_y_continuous(trans='log2') + geom_hline(yintercept = 50) 
```
...
```
  ...
  Are older people spending more per transaction?
